...
기능적 요구사항
1. 학생이 선생님을 선택하여 매칭 요청한다.
1. 학생이 결제한다.
1. 매칭 요청이 되면, 요청 내역이 선생님에게 전달된다 
1. 선생님이 요청을 확인한 후 방문일정이 잡힌다. 
1. 학생이 매칭을 취소할 수 있다.
1. 매칭요청이 취소되면 방문이 취소된다.
1. 학생이 매칭 상태를 중간중간 조회한다. 
1. 방문이 완료되면 상태가 변경된다.  
1. 매칭상태가 바뀔 때 마다 카톡으로 알림을 보낸다. (매칭요청, 방문일정셋팅, 방문완료, 매칭취소 등)

비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 매칭건은 아예 매칭이 성립되지 않아야 한다. Sync 호출
1. 장애격리
    1. 선생님 관리 기능이 수행되지 않더라도 매칭요청은 365/24 받을 수 있어야 한다. Async(event-driven) Eventual Consistency
1. 성능
    1. 학생이 매칭시스템에서 확인할 수 있는 상태를 마이페이지(프론트엔드)에서 확인할 수 있어야 한다 CQRS
    1. 매칭상태가 바뀔때마다 카톡 등으로 알림을 줄 수 있어야 한다 Event driven
...
![unnamed](https://user-images.githubusercontent.com/66051393/103783555-39c04200-507c-11eb-9988-639612963ecc.jpg)
